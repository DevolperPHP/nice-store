...
export default function ProductsAnalysis() {
  ...
  useEffect(async () => {
    const res = await axios(
      `${process.env.REACT_APP_SERVER_URL}/graph/products/sales?date=current-month`,
      { withCredentials: true }
    );

    if (res.data.done) setProductsSales(res.data.data);
    const res2 = await axios(
      `${process.env.REACT_APP_SERVER_URL}/graph/products/sales/in/months?date=current-month`,
      { withCredentials: true }
    );
