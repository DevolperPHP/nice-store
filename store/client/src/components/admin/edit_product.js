...
export default function EditProduct({ match }) {
    ...
    const handleRemoveSlider = (e) => {
        e.preventDefault();
        const url = `${process.env.REACT_APP_SERVER_URL}/remove/slider/${id}`;
        axios.put(url, {}, { withCredentials: true }).then(res => res.data.done && window.location.reload(true))
    }
    return (
        <section className="add-product-section">
            <div className="max-add-product">
                ...
                <div style={{ display: 'flex', width: '70%' }}>
                    ...
                    <form onClick={cancleDiscount}>
                        <div className="fileds">
                            <label htmlFor="">
                                <p>Discount Percenteg</p>
                                <input type="number" min="1" onChange={(e) => setDiscountPercentage(Number(e.target.value))} />
                                <br />
                                <button style={{ marginTop: '20px' }} onClick={addDiscount}>Make Discount</button>
                                {
                                    checkDiscount
                                        ?
                                        <button
