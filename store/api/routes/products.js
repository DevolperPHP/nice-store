...
const getOrSetCachList = require('./getOrSetCachList')
//GET ALL PRODUCTS
router.get('/products', async (req, res) => {
    try {
        ...
        if (sort == 'high') {
            const products = await getOrSetCachList(`products?${sort}`, 600, async () => {
        ...
        if (sort == 'discount') {
            ...
            const products = await getOrSetCachList(`products`, 3600, async () => {
...
router.get('/product/:path', async (req, res) => {
    try {
        ...
        const product = await getOrSetCachList(`product:${path}`, 3600, async () => {
            return await Product.findOne({ path })
        });
        if (!product) return res.send({ errMsg: 'No products with This path', done: false });
        const relatedProducts = await getOrSetCachList(`relatedProducts?${product.category}`, 3600, async () => {
            return await Product.find({ category: product.category, path: { $ne: path } }).select('-userQty -checkBuyWithPoints')
        });
...
router.post('/add/product', async (req, res) => {
    try {
        ...
        redisClinet.rpush('products', JSON.stringify([await Product.findOne({ path })]));
...
router.put('/discount/product/:id', async (req, res) => {
    ...
    const productId = req.params.id;
    const { price } = binaryId(productId, await Product.find());
