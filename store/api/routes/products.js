...
const getProductsSales = require("./getProductsSales");
...
router.get('/products', async (req, res) => {
    try {
        ...
        if (sort === 'high') {
            const products = mergeHigh(await Product.find({ active: true }).select('-userQty -checkBuyWithPoints'))
            return res.send(products)
        }
        if (sort === 'low') {
            const products = mergeLow(await Product.find({ active: true }).select('-userQty -checkBuyWithPoints'))
            return res.send(products)
        }
        if (sort === 'discount') {
            const products = await Product.find({ active: true, discount: true }).select('-userQty -checkBuyWithPoints')

            return res.send(products)
        }
        else {
            const products = await Product.find({ active: true }).select('-userQty -checkBuyWithPoints')
            return res.send(products)
        }
    } catch (err) {
        console.log(err)
...
router.get('/graph/products/sales', async (req, res) => {
    try {
        ...
        if (requestedDate === '1') return res.send({ done: true, products: await getProductsSales(1) })

        if (requestedDate === '2') return res.send({ done: true, products: await getProductsSales(2) })

        if (requestedDate === '5') return res.send({ done: true, products: await getProductsSales(5) })

        if (requestedDate === '12') return res.send({ done: true, products: await getProductsSales(12) })

        if (requestedDate === '24') return res.send({ done: true, products: await getProductsSales(24) })
        else {
