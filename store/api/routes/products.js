...
router.get('/products', async (req, res) => {
    try {
        ...
        if (sort === 'high') {
            const products = mergeHigh(await Product.find({ active }).select('-userQty -checkBuyWithPoints'))
            return res.send(products)
        }
        if (sort === 'low') {
            const products = mergeLow(await Product.find({ active }).select('-userQty -checkBuyWithPoints'))
            return res.send(products)
        }
        if (sort === 'discount') {
            const products = await Product.find({ active, discount: true }).select('-userQty -checkBuyWithPoints')

            return res.send(products)
        }
        else {
            const products = await Product.find({ active }).select('-userQty -checkBuyWithPoints')
            return res.send(products)
        }
    } catch (err) {
        console.log(err)
...
router.get('/product/:path', async (req, res) => {
    try {
        ...
        const relatedProducts = await Product.find({ category: product.category, path: { $ne: path }, active: true }).select('-userQty -checkBuyWithPoints')
...
router.get('/graph/products/sales', async (req, res) => {
    try {
        ...
        if (requestedDate === '1') return res.send({ done: true, data: await getIndividualData(1) });

        if (requestedDate === '2') return res.send({ done: true, data: await getIndividualData(2) });

        if (requestedDate === '5') return res.send({ done: true, data: await getIndividualData(5) });

        if (requestedDate === '12') return res.send({ done: true, data: await getIndividualData(12) });

        if (requestedDate === '24') return res.send({ done: true, data: await getIndividualData(24) });
        else {
...
router.get("/graph/products/sales/in/months", async (req, res) => {
    ...
    if (requestedDate === '2') {
        const data = await getLastMonthsData(2, 'Product');
        res.send({ data, done: true });
    }
    if (requestedDate === '5') {
        const data = await getLastMonthsData(5, 'Product');
        res.send({ data, done: true });
    }
    if (requestedDate === '12') {
        const data = await getLastMonthsData(12, 'Product');
        res.send({ data, done: true });
    }
    if (requestedDate === '24') {
        const data = await getLastMonthsData(24, 'Product');
        res.send({ data, done: true });
    }
})
...
router.post('/add/product', async (req, res) => {
    try {
        ...
        await new Product({
            ...
            score,
            discountScoreActive,
            discountScore: { points: Number(discountScorePoints), percentage: Number(discountScorePercentage) },
            addedScore,
            path,
