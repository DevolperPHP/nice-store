...
router.put('/edit/account', async (req, res) => {
    try {
        ...
        if (!password) return res.send({done:false, errMsg: 'Password Required'});
        const checkPassword = await bcrypt.compare(password, user.password);
        if (!checkPassword) return res.send({ done: false, errMsg: 'Invalid Password' });

        //MAKE THE QUERY
        let query = { $set: {} };
        const newUserData = _.pick(req.body, ['name', 'email', 'phone', 'governorate', 'address'])
        for (let key in newUserData) if (user[key] !== newUserData[key]) query.$set[key] = newUserData[key];
