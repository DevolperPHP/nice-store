...
router.post('/register', async (req, res) => {
    try {
        ...
        const { data } = await axios.post(`${process.env.CLIENT_URL}/send/verify/email`, { email }, { withCredentials: true });
        if (data.done) {
            return res.send({ done: true, succMsg: 'Verify Link Sendded to Your Email' });
        }
        else {
            return res.send({ done: false, errMsg: data.errMsg });
        }
    } catch (err) {
        console.log(err)
    }
